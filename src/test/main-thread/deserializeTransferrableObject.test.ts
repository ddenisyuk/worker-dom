...
test('Deserializes int arguments', (t) => {
  ...
  buffer.appendUint8(TransferrableObjectType.SmallInt);
  buffer.appendInt32(1);

  const deserializedArgs = deserializeTransferableMessage(new BytesStream(buffer.internal), stringContext, nodeContext, objectContext);
...
test('Deserializes float arguments', (t) => {
  ...
  buffer.appendUint32(1);
  buffer.appendUint8(TransferrableObjectType.Float);
...
test('Deserializes array argument', (t) => {
  ...
  buffer.appendUint8(TransferrableObjectType.SmallInt);
  buffer.appendInt32(args[0]);
  buffer.appendUint8(TransferrableObjectType.SmallInt);
  buffer.appendInt32(args[1]);
  buffer.appendUint8(TransferrableObjectType.SmallInt);
  buffer.appendInt32(args[2]);
